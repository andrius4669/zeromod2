...
void retrieveservers(int m, vector<char> &data)
{
    ENetSocket sock = connectmaster(m, true);
    if(sock == ENET_SOCKET_NULL) return;

    defformatstring(text)("retrieving servers from %s... (esc to abort)", getmasteraddress(m));
    renderprogress(0, text);
...
VARP(allowmasterserverscripts, 0, 1, 1);

void updatefrommaster()
{
    vector<char> data;
    extern int nummasterservers();
    int n = nummasterservers();
    loopi(n)
    {
        int oldlen = data.length();
        retrieveservers(i, data);
        if(data.length() <= oldlen) conoutf("master server (%s) not replying", getmasteraddress(i));
        else for(int j = data.length()-1; j >= 0; j--) if(data[j] == '\0') data[j] = '\n';
    }
    if(data.length())
    {
        clearservers();
        if(allowmasterserverscripts)
        {
            if(data.last() != '\0') data.add('\0');
            execute(data.getbuf());
        }
        else
        {
            char *input = data.getbuf(), *end = (char *)memchr(input, '\n', data.length());
            while(end)
            {
                *end++ = '\0';
                ...
                password[0] = '\0';
                int port = 0;
                if(sscanf(input, "addserver %100s %d %255s", name, &port, password) >= 1) addserver(name, port, password[0] ? password : NULL);
                int processed = end - data.getbuf();
                if(processed >= data.length()) break;
                input = end;
