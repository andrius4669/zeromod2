...
const char *getclienthostname(int n) { return clients.inrange(n) ? clients[n]->hostname : NULL; }
...
{
    if(m < 0) { loopv(masterservers) disconnectmaster(i); server::masterdisconnected(m); return; }

    if(masterservers[m].mastersock != ENET_SOCKET_NULL)
    {
        server::masterdisconnected(m);
        enet_socket_destroy(masterservers[m].mastersock);
        masterservers[m].mastersock = ENET_SOCKET_NULL;
    }

    masterservers[m].masterout.setsize(0);
    masterservers[m].masterin.setsize(0);
    masterservers[m].masteroutpos = masterservers[m].masterinpos = 0;

    masterservers[m].masteraddress.host = ENET_HOST_ANY;
    masterservers[m].masteraddress.port = ENET_PORT_ANY;

    masterservers[m].lastupdatemaster = 0;
...
{
    if(!masterservers.inrange(m) || !masterservers[m].mastername[0]) return ENET_SOCKET_NULL;
    if(masterservers[m].masteraddress.host == ENET_HOST_ANY)
    {
        if(isdedicatedserver()) logoutf("looking up %s...", masterservers[m].mastername);
        masterservers[m].masteraddress.port = masterservers[m].masterport;
        if(!resolverwait(masterservers[m].mastername, &masterservers[m].masteraddress)) return ENET_SOCKET_NULL;
    }
    ...
    if(wait || serveraddress.host == ENET_HOST_ANY || !enet_socket_bind(sock, &serveraddress))
    {
        ...
        if(wait)
        {
            if(!connectwithtimeout(sock, masterservers[m].mastername, masterservers[m].masteraddress)) return sock;
        }
        else if(!enet_socket_connect(sock, &masterservers[m].masteraddress)) return sock;
    }
    enet_socket_destroy(sock);
    if(isdedicatedserver()) logoutf("could not connect to master server (%s)", getmasteraddress(m));
    return ENET_SOCKET_NULL;
}
...
bool requestmaster(int m, const char *req)
{
    if(masterservers[m].mastersock == ENET_SOCKET_NULL)
    {
        masterservers[m].mastersock = connectmaster(m, false);
        if(masterservers[m].mastersock == ENET_SOCKET_NULL) return false;
        masterservers[m].lastconnectmaster = masterservers[m].masterconnecting = totalmillis ? totalmillis : 1;
    }

    if(masterservers[m].masterout.length() >= 4096) return false;

    masterservers[m].masterout.put(req, strlen(req));
    return true;
}
...
{
    if(masterservers[m].masterinpos >= masterservers[m].masterin.length()) return;

    char *input = &masterservers[m].masterin[masterservers[m].masterinpos],
        *end = (char *)memchr(input, '\n', masterservers[m].masterin.length() - masterservers[m].masterinpos);
    while(end)
    {
        *end++ = '\0';
        ...
        if(!strncmp(input, "failreg", cmdlen))
            conoutf(CON_ERROR, "master server (%s) registration failed: %s", getmasteraddress(m), args);
        else if(!strncmp(input, "succreg", cmdlen))
            conoutf("master server (%s) registration succeeded", getmasteraddress(m));
        else server::processmasterinput(m, input, cmdlen, args);

        masterservers[m].masterinpos = end - masterservers[m].masterin.getbuf();
        input = end;
        end = (char *)memchr(input, '\n', masterservers[m].masterin.length() - masterservers[m].masterinpos);
    } 

    if(masterservers[m].masterinpos >= masterservers[m].masterin.length())
    {
        masterservers[m].masterin.setsize(0);
        masterservers[m].masterinpos = 0;
    }
}

void flushmasteroutput(int m = -1)
{
    if(m < 0) { loopv(masterservers) flushmasteroutput(i); return; }

    if(masterservers[m].masterconnecting && totalmillis - masterservers[m].masterconnecting >= 60000)
    {
        logoutf("could not connect to master server (%s)", getmasteraddress(m));
        disconnectmaster(m);
    }
    if(masterservers[m].masterout.empty() || !masterservers[m].masterconnected) return;

    ENetBuffer buf;
    buf.data = &masterservers[m].masterout[masterservers[m].masteroutpos];
    buf.dataLength = masterservers[m].masterout.length() - masterservers[m].masteroutpos;
    int sent = enet_socket_send(masterservers[m].mastersock, NULL, &buf, 1);
    if(sent >= 0)
    {
        masterservers[m].masteroutpos += sent;
        if(masterservers[m].masteroutpos >= masterservers[m].masterout.length())
        {
            masterservers[m].masterout.setsize(0);
            masterservers[m].masteroutpos = 0;
        }
    }
...
{
    if(m < 0) { loopv(masterservers) flushmasterinput(i); return; }

    if(masterservers[m].masterin.length() >= masterservers[m].masterin.capacity())
        masterservers[m].masterin.reserve(4096);

    ENetBuffer buf;
    buf.data = masterservers[m].masterin.getbuf() + masterservers[m].masterin.length();
    buf.dataLength = masterservers[m].masterin.capacity() - masterservers[m].masterin.length();
    int recv = enet_socket_receive(masterservers[m].mastersock, NULL, &buf, 1);
    if(recv > 0)
    {
        masterservers[m].masterin.advance(recv);
...
void checkserversockets()        // reply all server info requests
{
    ...
    loopv(masterservers) if(masterservers[i].mastersock != ENET_SOCKET_NULL)
    {
        maxsock = max(maxsock, masterservers[i].mastersock);
        ENET_SOCKETSET_ADD(readset, masterservers[i].mastersock);
        if(!masterservers[i].masterconnected) ENET_SOCKETSET_ADD(writeset, masterservers[i].mastersock);
    }
    ...
    loopv(masterservers) if(masterservers[i].mastersock != ENET_SOCKET_NULL)
    {
        if(!masterservers[i].masterconnected)
        {
            if(ENET_SOCKETSET_CHECK(readset, masterservers[i].mastersock) || ENET_SOCKETSET_CHECK(writeset, masterservers[i].mastersock))
            { 
                int error = 0;
                if(enet_socket_get_option(masterservers[i].mastersock, ENET_SOCKOPT_ERROR, &error) < 0 || error)
                {
                    logoutf("could not connect to master server (%s)", getmasteraddress(i));
                    disconnectmaster(i);
                }
                else
                {
                    masterservers[i].masterconnecting = 0;
                    masterservers[i].masterconnected = totalmillis ? totalmillis : 1;
...
void updatemasterserver(int m = -1)
{
    if(m < 0) { loopv(masterservers) updatemasterserver(i); return; }
    if(!masterservers[m].masterconnected && masterservers[m].lastconnectmaster && totalmillis-masterservers[m].lastconnectmaster <= 5*60*1000) return;
    if(masterservers[m].mastername[0] && masterservers[m].allowupdatemaster) requestmasterf(m, "regserv %d\n", serverport);
    masterservers[m].lastupdatemaster = totalmillis ? totalmillis : 1;
}
...
void serverslice(bool dedicated, uint timeout)   // main server update, called from main loop in sp, or from below in dedicated server
{
    ...
    loopv(masterservers)
        if(!masterservers[i].lastupdatemaster || totalmillis-masterservers[i].lastupdatemaster>60*60*1000)  // send alive signal to masterservers every hour of uptime
...
static void setupwindow(const char *title)
{
    ...
    AppendMenu(appmenu, MF_STRING, MENU_RELOAD, "Reload configuration");
...
volatile bool restartserver = false;
...
void startlistenserver(int *usemaster)
{
    ...
    int allowupdatemaster = *usemaster>0 ? 1 : 0;
    int num = masterservers.length();
    if(num <= 1)
    {
    ...
    else
    {
        num = 0;
        allowupdatemaster = 0;
        loopv(masterservers) if(masterservers[i].allowupdatemaster) { allowupdatemaster = 1; num++; }
    }
    ...
    if(num <= 1) conoutf("listen server started for %d clients%s", maxclients, allowupdatemaster ? " and listed with master server" : "");
    else conoutf("listen server started for %d clients and listed with %d master servers", maxclients, num);
}
...
bool serveroption(char *opt)
{
    switch(opt[1])
    {
        ...
        {
            if(masterservers.empty()) masterservers.add();
            copystring(masterservers[0].mastername, opt+2);
            masterservers[0].allowupdatemaster = opt[2]!='\0';
            return true;
        }
#ifdef STANDALONE
