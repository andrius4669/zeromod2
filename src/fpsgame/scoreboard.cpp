    ...
    void renderscoreboard(g3d_gui &g, bool firstpass)
    {
        ...
        loopk(numgroups)
        {
            ...
            loopscoregroup(o, 
            {
                ...
                if(o->privilege)
                {
                    status = o->privilege>=PRIV_ADMIN ? 0xFF8000 : o->privilege>=PRIV_AUTH ? 0x40C0FF : 0x40FF80;
            ...
            if(showclientnum || player1->privilege>=PRIV_MASTER)
            {
                ...
                g.pushlist();
                g.text("cn", fgcolor);
                loopscoregroup(o, g.textf("%d", 0x40FFC0, NULL, o->clientnum));
                g.poplist();
            }
        ...
        if(showspectators && spectators.length())
        {
            if(showclientnum || player1->privilege>=PRIV_MASTER)
            {
                ...
                loopv(spectators) 
                {
                    ...
                    int status = 0xFFFFDD;
                    if(o->privilege) status = o->privilege>=PRIV_ADMIN ? 0xFF8000 : o->privilege>=PRIV_AUTH ? 0x40C0FF : 0x40FF80;
                ...
                g.pushlist();
                g.text("cn", 0xFFFF80);
                loopv(spectators) g.textf("%d", 0x40FFC0, NULL, spectators[i]->clientnum);
                g.poplist();
